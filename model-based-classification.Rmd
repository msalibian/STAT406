...
The goal is to classify a new observation at $\mathbf{x}_0 = (\mbox{Volume}, \mbox{Rate})^\top = (1.3, 1.9)^\top$. 
...
per class. This is not necessary, but it results in shorter and easier to read
code. 
...
(without the $\sqrt{2 \, \pi}$ that is common to both terms):
```{r lda1}
x0 <- c(1.9, 1.3)
p <- 2
