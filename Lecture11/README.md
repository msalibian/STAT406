...
Lecture slides are [here](STAT406-19-lecture-11.pdf).
...
Finally, we can check the predictions of the pruned tree on the test
set:
...
seem to be better than those of the tree pruned with the `rpart`
package. **Does this mean that `rpart` gives trees with worse
predictions than `tree` for data coming from the process than generated
our training set?** **Or could it all be an artifact of the specific
test set we used?** **Can you think of an experiment to check this?**
Again, it would be a **very good exercise** for you to check which fit
(`tree` or `rpart`) gives pruned trees with better prediction properties
in this case.
