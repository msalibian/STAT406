...
and a slightly regularized solution:
```{r isolate31}
set.seed(123)
a1 <- nnet(V618 ~ ., data=x.tr, size=6, decay=0.05, maxit=500, MaxNWts=4000, trace=FALSE)
a1$value
...
truth <- x.te$V618 <- as.factor(x.te$V618)
```
Use 6 units on the hidden layer and moderate
regularization via a decaying factor of `0.3` and a limit 
of `4000` on the
number of weights:
```{r isolate40.1}
set.seed(123)
...
mean(b2 != x.te$V618)
```
Did these solutions converge? A 0 value means convergence, while a 1
means that the maximum number of iterations was reached:
For `a1`: `r a1$convergence` and for `a2`: `r a2$convergence`.
